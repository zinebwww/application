...
<head>
    ...
    <style>
        :root { --primary: #6667AAE; --secondary: #764ba2; --success: #28a745; --danger: #dc3545; --warning: #ffc107; }
    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <div style="overflow-x: auto;">
                <table>
                    <thead>
                        <tr><th>Employé</th><th>Dates</th><th>Motif</th><th>Statut</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td><strong><?= htmlspecialchars($a['prenom'] . ' ' . $a['nom']) ?></strong><br><small><?= htmlspecialchars($a['departement']) ?></small></td>
                                <td><small><?= date('d/m/Y', strtotime($a['date_debut'])) ?> → <?= date('d/m/Y', strtotime($a['date_fin'])) ?></small></td>
                                <td><?= htmlspecialchars($a['motif']) ?></td>
                                <td><span class="badge badge-<?= $a['statut'] === 'Approuvé' ? 'success' : ($a['statut'] === 'En attente' ? 'warning' : 'danger') ?>"><?= $a['statut'] ?></span></td>
                                <td>
                                    <?php if ($a['statut'] === 'En attente'): ?>
                                        <form method="POST" style="display:inline"><input type="hidden" name="action" value="approuver"><input type="hidden" name="absence_id" value="<?= $a['id'] ?>"><button type="submit" style="width:auto;padding:2px 8px;">✓</button></form>
                                        <form method="POST" style="display:inline"><input type="hidden" name="action" value="rejeter"><input type="hidden" name="absence_id" value="<?= $a['id'] ?>"><button type="submit" style="width:auto;padding:2px 8px;background:var(--danger)">✗</button></form>
                                    <?php else: ?>—<?php endif; ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
